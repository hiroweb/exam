...
// --------------------------------------------------
	...
	// ログイン
	public function login($customer_ID=null,$disp_name=null){
		$now = time();
		$this->set('lastreq', $now);			// アクセス時刻を保存
		$this->set('customer_ID', $customer_ID);	// 顧客IDを保存
	...
	public function start($token=null){
		...
		$customer_ID = $this->get('customer_ID');		// IDチェック
		if (!$customer_ID) {
			$this->over();							// セッション破棄
			$this->msgcd = 'S01';
			return false;
		...
		if ($lastreq < $now - 7) {			// ダブルクリック以外の場合、セッション引越し
			...
			$this->set('lastreq', $now);			// アクセス時刻を保存
			$this->set('customer_ID', $customer_ID);	// 顧客ID引越しOUT
