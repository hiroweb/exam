...
// --------------------------------------------------
	...
	// ログイン
	public function login($merchant_ID=null,$disp_name=null){
		$now = time();
		$this->set('lastreq', $now);			// アクセス時刻を保存
		$this->set('merchant_ID', $merchant_ID);		// メンバーIDを保存
		$this->set('disp_name', $disp_name);	// メンバー名を保存
		if (LOG_file) $this->logging($merchant_ID,'Log in');		// ロギング
		return true;
	}
	...
	public function start($token=null){
		...
		$merchant_ID = $this->get('merchant_ID');		// IDチェック
		if (!$merchant_ID) {
		...
		if ($lastreq < $now - 7) {				// ダブルクリック以外の場合、セッション引越し
			...
			$this->set('lastreq', $now);			// アクセス時刻を保存
			$this->set('merchant_ID', $merchant_ID);		// メンバーID引越しOUT
	...
	public function over($REC='Unknown out'){
		if (LOG_file) {
			$this->logging($this->get('merchant_ID'),$REC);		// ロギング
		}
